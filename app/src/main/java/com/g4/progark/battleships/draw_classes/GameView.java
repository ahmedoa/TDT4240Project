...
public class GameView extends SurfaceView implements SurfaceHolder.Callback{
    ...
    private GameMode chosenGameMode = new StandardGameMode();
    ...
    public GameView(Context context,String map_name, GridView current_ship_grid, GridView current_strike_grid,
                    GridView opponent_ship_grid, int currentPlayer, int currentFirepowerInt, int chosenGameModeInt) {
        super(context);
        ...
        this.currentFirepowerInt = currentFirepowerInt;
        currentFirepower = firePowerFactory.setFirePower(0);
        //chosenGameMode = setGameMode(chosenGameModeInt);

    }
    public GameMode setGameMode(int chosenGameModeInt) {

        switch (chosenGameModeInt) {
            case 0:
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //drawGameView();
        currentFirepower = firePowerFactory.setFirePower(currentFirepowerInt);

        if(!isWithinPositionInStrikeGrid(event.getX(), event.getY())){
            if((currentFirepowerInt == 0))
                getFirepower();
            return false;
        }
    ...
    public void startWinningScreen() {
        ...
        Log.d("ok", "PLAYER " + currentPlayer +  " WON THE GAME");
